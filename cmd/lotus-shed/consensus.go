...
func runSlashing(ctx context.Context, from address.Address, fPath string, apiFull api.FullNode) error {
	...
	for {
		...
		minfo, err := apiFull.StateMinerInfo(ctx, slashC.Miner, types.EmptyTSK)
		if err != nil {
		...
		msg := &types.Message{
			...
			Params:     enc,
			GasPremium: types.NewInt(2500000),
		}

		log.Infow("slashing", "toSlash", slashC)
		smsg, err := apiFull.MpoolPushMessage(ctx, msg, &api.MessageSendSpec{
			MaxFee: types.BigInt(types.MustParseFIL("1")),
		})
		if err != nil {
			log.Warnf("sending message: %v", err)
			continue
		}
		msgRes, err := apiFull.StateWaitMsg(ctx, smsg.Cid(), 1)
		if err != nil {
			return err
		}
